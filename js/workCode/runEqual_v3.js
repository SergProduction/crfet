...
//----------------------------
function* generatorXlsxInBase(data, file){
  for(let xl of data){
    let rowXl = modXlsx(xl, args.splitXlsx)
     for(let row of base){
       ...
       if(usTrue){
          ...
          yield {source: copyPath.source, target: copyPath.target}
       }
     }
  }
}
...
function readXlsx(file){
    ...
    convertXslxTo(pathXlsx, (type, data) => {
      ...
      if(type == 'data'){
        ...
        iterXlsxInBase = generatorXlsxInBase(data, file)
        startXlsx()
      }
    })
}
...
function startXlsx(){
  ...
  myCopy(data.value.source, data.value.target, true)
}
...
function* generatorBase(file){
    ...
    for(let i=0; i<base.length; i++){
      let modDirFile = splitDirFile(file.file, args.splitFile, file.dir, args.splitDir)
      let usTrue = userEqual(base[i], args.splitBase, modDirFile, args.equalBaseFile, args.equalBaseDir)
      if(usTrue){
        
        newName.file = newNameSplit(base[i], file.file, args.endFile),
        newName.dir  = newNameSplit(base[i], file.dir , args.endDir )
