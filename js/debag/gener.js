...
function start(base, xlsxCol){
  let iterFile = generatorFileInFolder(args.userPath.source, args.dirInDir)
  if(
  !Array.isArray(base) ||
  ...
  if(!xlsxCol && base)
  for( let row of base ){
    let uif = userEqual(row, args.splitBase, args.equalBaseFile, args.equalBaseDir)
    if( uif){
      let newName = newNameFolderAndFile(row, oldName, args.endDir, args.endFile)
  ...
  for( let row of base ){
    for( let cell of xlsxCol ){
      ...
      if( uif){
        let newName = newNameFolderAndFile(row, oldName, usRenameFolred, usRenameFile)
        renameFileOrFolred(newName)
      }
    }
  }
...
function newNameFolderAndFile(row, oldName, usRenameFolred, usRenameFile){//текущая строка, новое имя дир, новое имя файла
...
function userEqual(row, usRowSplitVal, usTabRowNameFileVal, usTabRowNameFolderVal) {//текущая строка, разделитель строк в таблице, сравнение таблици с именем файла
  ...
  if( !usRowSplit[0]         ) usRowSplit         = []
  if( !usTabRowNameFile[0]   ) usTabRowNameFile   = []
  if( !usTabRowNameFolder[0] ) usTabRowNameFolder = []
  try{
    let rowMod = row;
    let result = [];
